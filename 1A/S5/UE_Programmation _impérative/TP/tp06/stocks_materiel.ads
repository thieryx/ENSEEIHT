...
package Stocks_Materiel is
    ...
    type T_Fonctionnement is (SERVICE, HORS_SERVICE);
    
    type T_materiel is record
        Numero_Serie: Integer;
        Nature: T_Nature;
        Annee_achat: Integer;
        Etat: T_Fonctionnement;
    end record;
    ...
    -- Obtenir le nombre de matériels dans le stock hors d'état de fonctionnement
    --
    -- Paramètres:
    --    Stock:  le stock à partir duquel on compte le nbre de matériels hors d'état de fonctionnement
    --
    -- Nécessite
    --    Vrai
    --
    -- Assure
    --    Resultat >=0 Et Résultat <=CAPACITE
    --
    ...
    -- Assure
    --     L'état du matériel est mis à jour
    --
    procedure Maj_Materiel(Stock: in out T_Stock;
                           Numero_Serie: in Integer
                          );
    ...
    -- Paramètres:
    --    Stock: le stock concerné
    --    Numero_Série: le numéro de série du matériel
    --
    -- Nécessite
    --    Vrai
    --
    ...
    --    Nb_Materiels (Stock) = Nb_Materiels (Stock)'Avant - 1
    --
    procedure Suppr_Materiel(Stock: in out T_Stock;
                             Numero_Serie: in Integer
                            ) with
            Post => Nb_Materiels (Stock) = Nb_Materiels (Stock)'Old - 1;
