...
procedure Evaluer_Alea_TH is
    ...
    is
		package Mon_Alea is
         ...
         function Hachage1(Cle: in Integer) return Integer is
         begin
             return Cle;
         end Hachage1;

         package TH1 is
                new TH(T_Cle => Integer, T_Donnee => Integer, borne_max => 1000, Hachage => Hachage1);
         use TH1;
    ...
    begin
        ...
        -- Création du TH avec la clé et la fréquence associée
        for i in 1..Taille loop
            Get_Random_Number(Nombre);
            if Cle_Presente(Sda, Nombre) then
                Enregistrer(Sda, Nombre, La_Donnee(Sda,Nombre)+1);
            else
                Enregistrer(Sda, Nombre, 1);
            end if;
        end loop;

        -- Recherche de la plus grande et de la plus petite fréquence
        for cle in 1..Borne loop
            if Cle_Presente(Sda,cle) then
                if La_Donnee(Sda, cle) > Max then
                    Max := La_Donnee(Sda, cle);
                elsif La_Donnee(Sda, cle) < Min then
                    Min := La_Donnee(Sda, cle);
                end if;
            end if;
        end loop;
...
exception
    when others => Put("Erreur lors de la saisie");
end Evaluer_Alea_TH;
