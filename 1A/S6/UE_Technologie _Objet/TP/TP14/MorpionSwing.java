...
// ---------------
	...
	public MorpionSwing(ModeleMorpion modele) {
        ...
        Image icone = Toolkit.getDefaultToolkit().getImage("croix.jpg");

		// Créer les cases du Morpion
		for (int i = 0; i < this.cases.length; i++) {
			for (int j = 0; j < this.cases[i].length; j++) {
				this.cases[i][j] = new JLabel(icone);
                this.cases[i][j].setBounds(100 + 50 * j, 100 + 50 * i, 50, 50);
                this.fenetre.setIconImage(icone);
        ...
        GridLayout grid = new GridLayout(4, 3, 1, 1);
        this.fenetre.getContentPane().setLayout(grid);
		for (int i = 0; i < this.cases.length; i++) {
			for (int j = 0; j < this.cases[i].length; j++) {
				this.fenetre.getContentPane().add(cases[i][j]);
        
        
        JMenuBar mb = new JMenuBar();
        JMenu jeu = new JMenu("jeu");
        jeu.add(this.boutonNouvellePartie);
        jeu.add(this.boutonQuitter);
        mb.add(jeu);
