...
int main() {
    ...
    sigpromask(SIG_BLOCK, &ens_signaux1, NULL);
    sigpromask(SIG_BLOCK, &ens_signaux2, NULL);
    ...
    /* démasquer SIGUSR1 */
    sigpromask(SIG_UNBLOCK, &ens_signaux1, NULL);

    /* attente de 10s */
    dormir(10);

    /* démasquer SIGINT */
    sigpromask(SIG_UNBLOCK, &ens_signaux2, NULL);
