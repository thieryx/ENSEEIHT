...
public class Interpolateur : MonoBehaviour
{
    ...
    (List<float>, List<float>) buildParametrisationDistance(int nbElem, float pas)
    {
        ...
        {
            T.Add(T.Last() + (float)Math.Sqrt(Math.Pow(X[i]-X[i-1],2) + Math.Pow(Y[i]-Y[i-1],2)));
        }
        // Construction des échantillons
        for (float j=0 ; j<=T.Last() ; j+=pas)
        {
    ...
    (List<float>, List<float>) buildParametrisationRacineDistance(int nbElem, float pas)
    {
        ...
        for (int i=1 ; i<nbElem ; i++)
        {
            T.Add(T.Last() + (float) Math.Sqrt(Math.Sqrt(Math.Pow(X[i]-X[i-1],2) + Math.Pow(Y[i]-Y[i-1],2))));
        }
        // Construction des échantillons
        for (float j=0 ; j<=T.Last() ; j+=pas)
        {
    ...
    (List<float>, List<float>) buildParametrisationTchebycheff(int nbElem, float pas)
    {
        ...
        for (int i=0 ; i<nbElem ; i++)
        {
            float val = (float) (((2*i+1)*Math.PI) / (2*(nbElem-1)+2));
            T.Add((float)(Math.Cos(val)));
            Debug.Log((float) Math.Cos(val));
        }
    ...
    {
        int n = X.Count;
        List<float> abscisses = new List<float>();
        List<float> ordonnees = new List<float>();
        ...
        for (int i=0; i<n-1; i++)
        {
            for (int j=0; j<n-i-1; j++)
            {
                ...
                float TG = (tg-t)/(tg-td);
                float TD = (t-td)/(tg-td);
                abscisses[j] = TG*abscisses[j]+TD*abscisses[j+1];
                ordonnees[j] = TG*ordonnees[j]+TD*ordonnees[j+1];
            }
        }
        return new Vector2(abscisses[0],ordonnees[0]);
    }
