...
public class Bernstein : MonoBehaviour
{
    ...
    void buildPolysBernstein() {
        List<Vector2> listBernstein = new List<Vector2>();
        for (int i = 0; i<=nombrePolynomesBernstein; i++)
        {
            long KpN = KparmiN(i,nombrePolynomesBernstein);
            for (float t=0.0f; t<=1.0f;t+=pas)
            {
                float val =  (float) KpN* (float) Math.Pow(1-t,nombrePolynomesBernstein-i)* (float) Math.Pow(t,i);
                listBernstein.Add(new Vector2(t,val));
            }
            ListePoints.Add(listBernstein);
        }
    }
