...
and ruleIf env cond bthen belse =
  let value = value_of_expr cond env in
    match value with
    | BooleanValue boolean -> if boolean then value_of_expr bthen env
...
and ruleFunction expr env =
  match expr with
  | FunctionNode (_, _) -> FrozenValue (expr, env)
  | _ -> ErrorValue UndefinedExpressionError
...
and ruleCallByName env fexpr pexpr =
  ...
  match fvalue with
  | FrozenValue (FunctionNode (ident, body), fenv) ->
    let pvalue = value_of_expr pexpr env in
    (match pvalue with
     | ErrorValue _ as result -> result
     | _ -> value_of_expr body ((ident, pvalue) :: fenv))
  (* A traiter*)
  | _ -> ErrorValue UndefinedExpressionError
...
and ruleCallByValue env fexpr pexpr =
  ...
  let fvalue = value_of_expr fexpr env in
  match fvalue with
  | FrozenValue (FunctionNode (ident, e3), fenv) ->
    let pvalue = value_of_expr pexpr env in
    (match pvalue with
     | ErrorValue _ as result -> result
     | _ -> value_of_expr e3 ((ident, pvalue) :: fenv))
  | _ -> ErrorValue UndefinedExpressionError
...
and ruleLetrec env ident bvalue bin =
  match value with
    |ErrorValue _ as result -> result
    | _ -> value_of_expr bin ((ident,FixPoint(vbalue,env));;env)
