...
type info =
  ...
  (* Information associée à une boucle loop : son nom (utile pour l'appel d'un break ou d'un continue) *)
  | InfoBoucle of string * string * string * info option
  (* Information associée à un if : son nom (utile pour l'appel d'un break ou d'un continue) *)
 (* Information associée à un break : le nom de la boucle loop auquel il s'applique *)
  | InfoBreak of string
  (* Information associée à un continue : le nom de la boucle loop auquel il s'applique *)
...
(* Recherche les informations de toutes les fonctions d'un même identificateur dans la tds globale *)
val chercherGlobalementFonctions : tds -> string -> info_ast list
...
val chercherBoucle : info option -> string -> info_ast option

val trouverDansTds : string -> info_ast option -> tds -> info_ast option

val filtrerTds : tds -> tds

val filtrerTdsContinueBreak : tds -> tds