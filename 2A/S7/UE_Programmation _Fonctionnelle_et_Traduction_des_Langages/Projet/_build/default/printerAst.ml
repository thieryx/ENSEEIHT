...
struct
  ...
  let rec string_of_instruction i =
    ...
    | Boucle (b) -> "loop" ^ ((List.fold_right (fun i tq -> (string_of_instruction i)^tq) b ""))^"\n"
    | BoucleID (n,b) -> n^" : loop" ^ ((List.fold_right (fun i tq -> (string_of_instruction i)^tq) b ""))^"\n"
    | BreakID (n) -> "Break  : BREAK "^n^"\n"
    | ContinueID (n) -> "Continue  : CONTINUE "^n^"\n"
    | Conditionnelle_opt (c,e) -> "Conditionnelle_opt  : IF "^(string_of_expression c)^"\n"^
    "THEN \n"^((List.fold_right (fun i tq -> (string_of_instruction i)^tq) e ""))
