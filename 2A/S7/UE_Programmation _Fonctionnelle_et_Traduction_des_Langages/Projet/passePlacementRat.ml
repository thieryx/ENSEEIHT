...
let rec analyse_placement_instr i depl reg =
  match i with
    ...
    | AstType.Conditionnelle_opt (e,b) ->
      let nb = analyse_placement_bloc b depl reg in
      (AstPlacement.Conditionnelle_opt (e,nb), 0)
    ...
    | AffichagePointeur e -> (AstPlacement.AffichagePointeur e, 0)
    | AstType.Empty -> (AstPlacement.Empty, 0)
    | AstType.BoucleID (ia,b) ->
      let nb = analyse_placement_bloc b depl reg in
      (AstPlacement.BoucleID (ia,nb), 0)
    | AstType.Boucle (ia,b) ->
      let nb = analyse_placement_bloc b depl reg in
      (AstPlacement.Boucle (ia,nb), 0)
    | AstType.BreakID (ia) -> (AstPlacement.BreakID (ia), 0)
    | AstType.Break (ia) -> (AstPlacement.Break (ia), 0)
    | AstType.ContinueID (ia) -> (AstPlacement.ContinueID (ia), 0)
