...
let%test _ = not(appartient_arbre (decompose_chaine "bas") (retrait_arbre (decompose_chaine "bas") arbre_sujet))

let parcours_arbre (Noeud(b,lb)) = 
   let rec aux (Noeud(b,lb)) l = 
      match lb with
      | [] -> if b then [l] else []
      | (c,a)::q -> (aux a (l@[c]))@(aux (Noeud(b,q)) l)
   in aux (Noeud(b,lb)) []

let elim_arbre (Noeud(b,lb)) =
   let aux (Noeud(b,lb)) list_mot nv_arbre=
       match lb with
       | [] -> if b then [l] else []
       | (c,a)::q -> List.map (fun l -> if List.map(fun a -> if a=c then))
   in aux (Noeud(b,lb)) []
