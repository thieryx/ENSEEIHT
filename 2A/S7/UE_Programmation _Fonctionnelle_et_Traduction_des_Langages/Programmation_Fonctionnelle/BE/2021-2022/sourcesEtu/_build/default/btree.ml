...
let rec height tr =
    match tr with
    | Leaf a -> 0
    | Node(d,g) -> 1 + max (height d) (height g)
...
let rec num_nodes tr =
    match tr with
    | Leaf a -> 1
    | Node(d,g) -> 1 + (num_nodes d) + (num_nodes g)
...
let rec num_values tr = 
    ...
    | Node(d,g) -> (num_values d) + (num_values g)
...
let rec bits n addr =
    ...
    | (0,_) ->  []
    | (n,0) -> [0]@(bits (n-1) 0)
    | (n,addr) -> if addr mod 2 = 1 then [1]@(bits (n-1) (addr/2))
    else [0]@(bits (n-1) (addr/2))
...
let rec search tr addrl =
    ...
    | (Leaf a, _) -> a
    | (Node(_,_), []) -> _0
    | (Node(g,d),t::q) -> if (t=1) then search d q
    else search g q
...
let rec sprout x addrl =
    match addrl with
    | [] -> Leaf x
    | t::q -> if (t=1) then Node(Leaf _0, sprout x q)
    else Node(sprout x q, Leaf _0)
...
let rec update tr x addrl = 
    match (tr,addrl) with
    | (Leaf a,[]) -> Leaf x
    | (Leaf _0, list) -> sprout x list
    | (Node(g,d), t::q) -> if (t=1) then Node(g, (update d x q))
                        else Node((update g x q), d)
