...
struct
  ...
  let get_branche e (Noeud l) = List.assoc_opt e l
  ...
  let  arbre_to_liste (Noeud l) =
    let rec aux (Noeud l) list = 
      match l with
      |[] -> []
      |(code,Lettre b)::q ->[b,list@[code]]@(aux (Noeud q) list)
      |(c,a)::q -> (aux a (list@[c]))@(aux (Noeud q) list)
  in aux (Noeud l) []

  let%test _ = arbre_to_liste arbre1 = [('e', [1; 1]); ('b', [1; 2]); ('c', [2; 1; 2]); ('f', [2; 1; 3]);
  ('d', [2; 2]); ('a', [3])]
  ...
  let chiffrer list arbre = let arb_to_l = arbre_to_liste arbre in
    List.flatten(List.map (fun t -> match List.assoc_opt t arb_to_l with
                                    | None -> raise DonneeNonValide
                                    | Some v -> v) list)
...
struct
  ...
  (* dechiffrer : code -> (symbole_code,symbole_donnee) arbre_chiffrement -> donnee
     ...
     Exception CodeNonValide : si le code ne peut pas être déchiffré avec l'arbre de chiffrement
   *)
  let dechiffrer code arbre = DRDonnee.recompose(Arbre.dechiffrer (DRCode.decompose code) arbre)
  ...
  (* chiffrer : donnee -> (symbole_code,symbole_donnee) arbre_chiffrement -> code
     ...
     Exception DonneeNonValide : si la donnéee ne peut pas être chiffrée avec l'arbre de chiffrement 
   *)
  let chiffrer donnee arbre = DRCode.recompose(Arbre.chiffrer (DRDonnee.decompose donnee) arbre)
