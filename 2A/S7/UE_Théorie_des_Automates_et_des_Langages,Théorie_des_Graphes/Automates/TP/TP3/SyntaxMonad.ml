...
and parseEX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | EqualToken ->
        ...
        parseEX)
    (* regle #7 *)
    | (RightParenthesisToken) | EOSToken | InToken | ThenToken | ElseToken->
        (print_endline "");
        (inject stream)
    | _ -> Failure)
...
and parseTX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | MinusToken ->
        ...
        parseTX)
    (* regle 11 *)
    | (RightParenthesisToken | EqualToken | EOSToken | InToken | ThenToken | ElseToken) -> 
        (print_endline "");
        (inject stream)
    | _ -> Failure)
...
and parseFX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | DivideToken ->
        ...
        parseFX)
    (* regle 15 *)
    | (RightParenthesisToken | EqualToken | PlusToken | MinusToken | EOSToken | InToken | ThenToken | ElseToken) ->
        (print_endline "");
        (inject stream)
    | _ -> Failure)
...
and parseARG stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    accept LeftParenthesisToken
    (* regle 26 *)
    | (TimesToken | DivideToken | PlusToken | MinusToken | EqualToken | IfToken | EOSToken | InToken | ThenToken | ElseToken)  ->
        (print_endline "");
        (Success stream)
    | _ -> Failure)
;;
