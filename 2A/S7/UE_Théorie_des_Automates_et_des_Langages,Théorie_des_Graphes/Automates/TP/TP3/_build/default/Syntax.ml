  ...
  *)
  ...
  (match (peekAtFirstToken stream) with
    ...
    | FunctionToken -> (match (accept FunctionToken stream) with
      | Success next -> (match (acceptIdent next) with
        | Success next2 -> (match (accept BodyToken next2) with
          | Success next3 -> (parseE next3)
          | _ -> Failure)
        | _ -> Failure)
      | _ -> Failure)
    ...
    | IfToken -> (match (accept IfToken stream) with
      | Success next -> (match (parseE next) with
        | Success next2 -> (match (accept ThenToken next2) with
          | Success next3 -> (match (parseE next3) with
            | Success next4 -> (match (accept ElseToken next4) with
...
and parseEX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    (match (accept EqualToken stream) with
       ...
       | _ -> Failure)
    (* regle #7  - EX ->          | { $, ) } *)
    | (EOSToken | RightParenthesisToken) ->
    (print_endline "");
    (Success stream)
    | _ -> Failure)
...
and parseTX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | _ -> Failure)
    (* regle #11 - TX ->          | { $, =, ) } *)
    | EOSToken | EqualToken | RightParenthesisToken -> (print_endline "");(Success stream)
    | _ -> Failure)
...
and parseFX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    |DivideToken -> (match (accept DivideToken stream) with
      ...
      | _ -> Failure)
    (* regle #15 - FX ->          | { $, +, -, =, ) } *)
    | EOSToken | PlusToken | MinusToken | EqualToken | RightParenthesisToken -> (print_endline "");(Success stream)
    | _ -> Failure)
